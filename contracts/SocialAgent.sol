...
contract Agent {
    ...
    function runAgent(string memory user_query_ , string memory query, uint8 max_iterations) public returns (uint) {
        AgentRun storage run = agentRuns[agentRunCount];
        run.user_query = user_query_;
    ...
    function onOracleFunctionResponse(
        ...
        if (!compareStrings(errorMessage, "")) {
           ...
           IOracle.Message memory userQuery =  createTextMessage("user", string(abi.encodePacked(run.user_query,"Result :",result)));
             run.messages.push(userQuery);
        run.responsesCount++;
