...
import ABI from "../abi/SocialAgent.json"; // Adjust the path as needed
...
async function main() {
  ...
  if (!contractAddress)
    throw new Error("Missing AGENT_CONTRACT_ADDRESS in .env");
  ...
  const userQuery = await getUserInput("User's query: ");
  ...
  if (agentRunID === undefined || agentRunID === null) {
    return;
  }
  ...
  for (const log of receipt.logs) {
    try {
      ...
      if (parsedLog && parsedLog.name === "AgentRunCreated") {
        ...
        agentRunID = Number(parsedLog.args[1]);
      }
    } catch (error) {
